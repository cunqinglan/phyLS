...
class phy_map_impl {
 ...
 public:
  ...
  map_ntk_t rudy_map_test() {
    std::cout << "Starting RUDY test" << std::endl;
    auto [res, old2new] = initialize_map_network();

    /* compute and save topological order */
    top_order.reserve( ntk.size() );
    topo_view<Ntk>( ntk ).foreach_node( [this](auto n) { top_order.push_back(n); 
    } );
 ...
 protected:
  void set_RUDY_map(std::vector<node_position>* placement_p,
                    map_ntk_t* binding_network_p, uint32_t num_pis,
                    uint32_t num_pos) {
    rudy_map = std::make_unique<phyLS::RUDY<map_ntk_t>>(
        placement_p, binding_network_p, static_cast<int>(num_pis),
        static_cast<int>(num_pos));
  }

  void calculateRUDY() { rudy_map->calculateRudy(); }
 ...
 protected:
  ...
  void search_nodes_pins(uint32_t n, CutType* cut, uint16_t level = 0) {
    ...
    ntk.foreach_fanin(ntk.index_to_node(n), [&](signal<Ntk> const& fi) {
      ...
      if (!in_leaves) {
        if (level > 5) {
          std::cerr << "Error: cut size in leaves = " << cut->size()
                    << std::endl;
          return;
        }

        // Add the father into the nodes collection when the node is between the
        // root and the leaves
        if (std::find(cut->nodes.begin(), cut->nodes.end(), n) !=
            cut->nodes.end())
          std::cerr << "The node " << child
                    << " has been already in the cut of " << n << std::endl;
        ...
        if (std::find(cut->pins.begin(), cut->pins.end(), n) != cut->pins.end())
          std::cerr << "The node " << child
                    << " has been already in the pins of " << n << std::endl;
        return;
      }
    });
  ...
  void match_wirelength_drop_phase(node<Ntk> const& n) {
    ...
    if (!use_zero && !use_one) {
      ...
      node_data.congest_flow[0] =
          node_data.congest_flow[0] / node_data.est_refs[0];
      node_data.congest_flow[1] =
          node_data.congest_flow[1] / node_data.est_refs[1];
      node_data.congest_flow[2] =
          node_data.congest_flow[0] + node_data.congest_flow[1];
  ...
  inline void set_match_wirelength_complemented_phase(uint32_t index,
    ...
    // Treat the congest flow as the area flow
    node_data.congest_flow[phase] =
        node_data.congest_flow[phase] / node_data.est_refs[2];
    ...
    ntk.foreach_pi([&](auto const& n) {
      ...
      if constexpr (PHYAWARE) {
        ...
        if (match_position.size() != dest.size())
          std::cerr << "Error: match_position size is not equal to dest size: "
                       "index = "
                    << ntk.node_to_index(n)
                    << " match size = " << match_position.size()
  ...
  void finalize_cover_congest(NtkDest& res, klut_map& old2new) {
    for (auto const& n : top_order) {
      ...
      if (index == 439)
        std::cout << "match size = " << match_position.size()
        ...
        if (node_data.map_refs[1] > 0) {
          ...
          if constexpr (PHYAWARE) {
            ...
            if (match_position.size() != res.size())
              std::cerr << "Error1: match_position size is not equal to res "
                           "size: index = "
                        << index << " match size = " << match_position.size()
                        << " res size = " << res.size() << std::endl;
          }
      ...
      if (node_data.same_match || node_data.map_refs[phase] > 0) {
        auto is_new = create_lut_for_gate<NtkDest>(res, old2new, index, phase);
        if constexpr (PHYAWARE) {
          if (is_new) {
            ...
            if (match_position.size() != res.size())
              std::cerr << "Error2: match_position size is not equal to res size: "
                          "index = "
                        << index << " match size = " << match_position.size()
                        << " res size = " << res.size() << std::endl;
          }
        ...
        if (node_data.same_match && node_data.map_refs[phase ^ 1] > 0) {
          ...
          if constexpr (PHYAWARE) {
            if (is_new) {
              ...
              if (match_position.size() != res.size())
                std::cerr << "Error3: match_position size is not equal to res "
                            "size: index = "
                          << index << " match size = " << match_position.size()
                          << " res size = " << res.size() << std::endl;
            }
      ...
      if (!node_data.same_match && node_data.map_refs[phase] > 0) {
        ...
        if constexpr (PHYAWARE) {
          if (is_new) {
            ...
            match_position.push_back(node_data.position[phase]);
            if (match_position.size() != res.size())
              std::cerr << "Error: match_position size is not equal to res size: "
                          "index = "
                        << index << " match size = " << match_position.size()
                        << " res size = " << res.size() << std::endl;
          }
  ...
  template <class NtkDest>
  bool create_lut_for_gate(NtkDest& res, klut_map& old2new, uint32_t index,
                           unsigned phase) {
    auto const& node_data = node_match[index];
    auto& best_cut = cuts.cuts(index)[node_data.best_cut[phase]];
...
binding_view<klut_network> phymap(
  ...
  map_stats st;
  detail::phy_map_impl<Ntk, CutSize, CutData, NInputs, Configuration> p{
